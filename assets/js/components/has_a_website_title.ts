...
interface CogAnimation {
    cog: SVGGElement;
    direction: 1 | -1;      // set to 1 if clockwise, -1 if anticlockwise 
    time: {
        start: number;
        duration: number;      // duration of 1 full rotation
    }
    centre: {
        x: number;
        y: number;
...
class HasAWebsiteTitle extends HTMLElement {
    ...
    private setupAnimations() {
        const cogs: [SpinDirection, string[]][] = [
            ["clockwise", [
                "cog-h-top", "cog-h-bottom", "cog-a1-top", "cog-s1-top", "cog-a2-top", "cog-a2-bottom", "cog-w", 
                "cog-e1-top", "cog-e1-bottom", "cog-b-top-small", "cog-b-middle", "cog-s2-top", "cog-s2-bottom", 
                "cog-i-middle", "cog-t-top", "cog-t-bottom"
            ]],
            ["anticlockwise", [
                "cog-a1-middle", "cog-a1-s1", "cog-a2-middle", "cog-w-e1", "cog-b-bottom", "cog-b-top-big", 
                "cog-b-s2", "cog-i-bottom", "cog-i-t", "cog-e2"
            ]]
        ];
        ...
        this.cogAnimations.forEach((cog) => cog.start())

        this.addEventListener("mouseenter", (e) => {
                this.cogAnimations.forEach((cog) => {cog.changeSpeed(6_000);})
            }
        )

        this.addEventListener("mouseleave", (e) => {
                this.cogAnimations.forEach((cog) => {cog.changeSpeed(20_000);})
            }
        )
    }
}
...
class CogAnimation {
    ...
    private tick = (now: number) => {
        ...
        const progress = (elapsedTime % this.time.duration) / this.time.duration;
