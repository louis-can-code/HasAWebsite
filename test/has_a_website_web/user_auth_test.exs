defmodule HasAWebsiteWeb.UserAuthTest do
  ...
  @remember_me_cookie "_has_a_website_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, HasAWebsiteWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    test "erases session and cookies", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == ~p"/"
      assert nil == Accounts.get_user_by_session_token(user_token)
    end
